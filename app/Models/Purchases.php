...
class Purchases extends Model
{
    ...
    public static function createOrUpdate($params, $method, $request)
    {
        ...
        if (isset($params['id']) && $params['id']) {
            $oldDetails = PurchaseDetails::where('purchase_id', $params['id'])->get();
            foreach ($oldDetails as $oldDetail) {
                Inventories::where('id', $oldDetail->inventory_id)
                    ->decrement('stock', $oldDetail->qty);
            }
        ...
        if ($number === 'Auto Generated' || empty($number)) {
            ...
            $number = 'PUR-' . \Carbon\Carbon::now()->format('Ymd') . '-' . str_pad($count + 1, 4, '0', STR_PAD_LEFT);
        }
