    ...
    <script>
        ...
        function validateStock(rowIdx) {
            ...
            if (item && qty > item.stock) {
                Swal.fire({
                    ...
                    text: 'Only ' + item.stock + ' unit(s) of ' + item.name + ' available.',
                    toast: true,
                    position: 'top-end',
                    timer: 4000,
        ...
        $('form#main-form').submit(function(e) {
            ...
            $('.detail-row').each(function() {
                let rowIdx = $(this).attr('id').replace('detail-row-', '');
                if (!validateStock(rowIdx)) {
                    hasStockError = true;
                }
            });
            ...
            if (hasStockError) {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'The quantity exceeds the available stock. Please correct it before saving.'
                });
                return;
            }
